	...
	int& id)
{
	...
	else
	{
		...
		noiseModel::Diagonal::shared_ptr gaussian_model = noiseModel::Diagonal::Variances(Vector6);
		noiseModel::Diagonal::shared_ptr loop_noise = noiseModel::Diagonal::Variances(Vector6);
		noiseModel::Base::shared_ptr robust_noise = 
		gtsam::noiseModel::Robust::Create(gtsam::noiseModel::mEstimator::Huber::Create(1.0), gaussian_model);
	...
	const int& loop_key1)
{
	...
	if(icp.hasConverged() == false || fitness_score > fitness_score_threshold_*1)
	{
...
void distributedMapping::performExternLoopClosure()
{
	...
	*scan_cloud_ds = *transformPointCloud(*scan_cloud_ds, loop_pose0);
	...
	Pose3 current_estimate_j = initial_values->at<Pose3>(loop_symbol1);
	Pose3 loop_implied_j = pose_from * pose_between; // Using the loop transformation

	double position_error = (current_estimate_j.translation() - loop_implied_j.translation()).norm();

	// 2. HARD REJECT if the jump is too big (e.g., > 15 meters)
	//    Unless your robots drift 15m in a few seconds, this is a false positive.
	if (position_error > 15.0) {
		ROS_WARN("REJECTED Loop: Implied correction %f m is too large (Teleportation detected)", position_error);
		return;
	}
	// get pose transformation
	...
	Vector Vector6(6);
	// Use the inflated noise. We trust rotation (first 3) more than translation (last 3) usually, 
	// but here we inflate everything to be safe.
	Vector6 << inflated_noise, inflated_noise, inflated_noise, inflated_noise, inflated_noise, inflated_noise;

	noiseModel::Diagonal::shared_ptr gaussian_model = noiseModel::Diagonal::Variances(Vector6);
	// Keep your Robust Kernel from the previous fix!
	noiseModel::Base::shared_ptr robust_noise = 
		gtsam::noiseModel::Robust::Create(gtsam::noiseModel::mEstimator::Huber::Create(1.0), gaussian_model);
	// noiseModel::Diagonal::shared_ptr loop_noise = noiseModel::Diagonal::Variances(Vector6);
	// add factor
